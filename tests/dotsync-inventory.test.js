...
if (!skip) {
  ...
  seed = function (p, body = 'x') { fs.mkdirSync(path.dirname(p), { recursive: true }); fs.writeFileSync(p, body); };
...
test('cmdInventory writes the file + returns 0', { skip }, () => {
  const logs = [];
