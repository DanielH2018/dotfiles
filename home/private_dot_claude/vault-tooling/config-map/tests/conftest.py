...
def fake_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> dict[str, Path]:
    ...
    for sub in ("hooks", "agents", "commands", "rules", "output-styles", "scheduled", "plugins", "docs"):
    ...
    (chezmoi_root / "hooks" / "executable_demo-hook.sh").write_text("#!/bin/bash\n# demo hook purpose\n")
    (claude_dir / "hooks" / "demo-hook.sh").write_text("#!/bin/bash\n# demo hook purpose\n")
    ...
    settings = {
        ...
        "outputStyle": "Terse",
        "effortLevel": "high",
        "enabledPlugins": {"demo-plugin@demo-market": True, "off-plugin@demo-market": False},
        "hooks": {
            "PreToolUse": [
                {
                    "matcher": "Bash",
                    "hooks": [{"type": "command", "command": "~/.claude/hooks/demo-hook.sh", "timeout": 10}],
                }
            ],
            "PostToolUse": [
                {"matcher": "Edit", "hooks": [{"type": "command", "command": "~/.claude/hooks/work-hook.sh"}]}
            ],
        },
    }
    ...
    (plugin_install / ".claude-plugin" / "plugin.json").write_text(
        json.dumps({"name": "demo-plugin", "description": "demo plugin does useful things"})
    )
        ...
        json.dumps(
            {
                "plugins": {
                    "demo-plugin@demo-market": [
                        {"scope": "user", "version": "1.0.0", "installPath": str(plugin_install)}
                    ],
    ...
    (claude_dir / "plugins" / "known_marketplaces.json").write_text(json.dumps({"demo-market": {}}))

    (claude_dir / "CLAUDE.md").write_text("# Main\n\n## Section A\n\ntext\n\n## Section B\n\n@~/.claude/docs/extra.md\n")
    ...
    plist = {
        ...
        "ProgramArguments": ["/bin/zsh", "-lc", "exec ~/.claude/scheduled/run-skill.sh healthcheck headless"],
        "StartCalendarInterval": [{"Weekday": w, "Hour": 8, "Minute": 15} for w in range(1, 6)],
    }
    ...
    claude_json.write_text(
        json.dumps({"mcpServers": {"grafana": {"command": "grafana-mcp", "token": "sk-should-not-leak"}}})
    )

    mcp_cache = claude_dir / "mcp-needs-auth-cache.json"
    mcp_cache.write_text(json.dumps({"claude.ai Demo Connector": {"id": "mcpsrv_secretvalue"}}))

    monkeypatch.setattr(scan, "HOME", home)
    ...
    monkeypatch.setattr(scan, "CLAUDE_JSON", claude_json)
    monkeypatch.setattr(scan, "MCP_NEEDS_AUTH_CACHE", mcp_cache)
    monkeypatch.setattr(scan, "MANAGED_SETTINGS", tmp_path / "absent-managed-settings.json")
    monkeypatch.setattr(scan, "PROJECT_SETTINGS_LOCAL", vault_root / ".claude" / "settings.local.json")
    monkeypatch.setattr(scan, "PROJECT_SETTINGS", vault_root / ".claude" / "settings.json")
    monkeypatch.setattr(scan, "PROJECT_CLAUDE_MD", vault_root / "CLAUDE.md")

    return {"home": home, "claude_dir": claude_dir, "chezmoi_root": chezmoi_root, "work_root": work_root, "vault_root": vault_root}
