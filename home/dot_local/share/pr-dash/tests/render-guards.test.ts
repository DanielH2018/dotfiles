...
// The launcher's own shape: 24 random bytes as 48 lowercase hex characters.
const LAUNCH_SECRET = 'a'.repeat(48);

test('resolveSecret takes the secret from the fragment and caches it for the bare URL', () => {
  const storage = fakeStorage();

  const resolved = resolveSecret(`#${LAUNCH_SECRET}`, storage);

  assert.strictEqual(resolved.secret, LAUNCH_SECRET);
  assert.strictEqual(resolved.fromHash, true, 'the caller strips the fragment only on this path');
  assert.strictEqual(storage.written.get(SECRET_KEY), LAUNCH_SECRET);
});

test('resolveSecret reads the cache back when the URL carries no fragment', () => {
  // This is what makes http://127.0.0.1:<port> bookmarkable: the launcher's open seeded
  // the cache, and a later visit to the clean URL has no fragment at all.
  const storage = fakeStorage();
  storage.written.set(SECRET_KEY, LAUNCH_SECRET);

  const resolved = resolveSecret('', storage);

  assert.strictEqual(resolved.secret, LAUNCH_SECRET);
  assert.strictEqual(resolved.fromHash, false, 'there is no fragment to strip');
});

test('resolveSecret neither caches nor returns a fragment that is not the launcher shape', () => {
  // An arbitrary fragment — someone else's link, a stale bookmark carrying junk — must not
  // put an attacker-chosen string into storage, and must not be sent as a credential.
  for (const junk of ['#nope', '#' + 'a'.repeat(47), '#' + 'a'.repeat(49), '#' + 'A'.repeat(48), '#../etc']) {
    const storage = fakeStorage();

    const resolved = resolveSecret(junk, storage);

    assert.strictEqual(resolved.secret, '', `expected ${junk} to yield no secret`);
    assert.strictEqual(resolved.fromHash, false);
    assert.strictEqual(storage.written.size, 0, `expected ${junk} not to be cached`);
  }
});

test('resolveSecret rejects a cached value of the wrong shape rather than sending it', () => {
  const storage = fakeStorage();
  storage.written.set(SECRET_KEY, 'tampered');

  assert.strictEqual(resolveSecret('', storage).secret, '');
});

test('resolveSecret survives a storage that throws on write, still returning the fragment', () => {
  // A private window cannot cache, so the bare URL will not work until the next launch —
  // but this visit has the secret in hand and must still load.
  const storage = fakeStorage({
    setItem: () => {
      throw new Error('blocked');
    },
  ...
  assert.strictEqual(resolveSecret(`#${LAUNCH_SECRET}`, storage).secret, LAUNCH_SECRET);
});

test('resolveSecret survives a storage that throws on read', () => {
  const storage = fakeStorage({
    getItem: () => {
      throw new Error('blocked');
    },
...
test('clearStoredSecret drops the secret and leaves the persisted view alone', () => {
  // Reset clears the view; a refused secret clears the secret. Neither may take the other,
  // or un-sticking a filter would log the page out and a dead credential would wipe the
  // user's filters.
  const storage = fakeStorage();
  storage.written.set(SECRET_KEY, LAUNCH_SECRET);
  storage.written.set(VIEW_KEY, '{"sort":"age"}');

  clearStoredSecret(storage);

  assert.strictEqual(storage.written.has(SECRET_KEY), false);
  assert.strictEqual(storage.written.get(VIEW_KEY), '{"sort":"age"}');
});

test('clearStoredSecret survives a storage that throws on removeItem', () => {
  const storage = fakeStorage({
    removeItem: () => {
      throw new Error('blocked');
    },
  ...
  assert.doesNotThrow(() => clearStoredSecret(storage));
});

test('relaunchBanner names the command that mints a new secret', () => {
  // The fix for this state is neither Refresh nor waiting — only a new launch mints a
  // secret — so the banner has to name the command.
  assert.match(relaunchBanner(), /\bpr-dash\b/);
});
