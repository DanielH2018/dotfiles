...
spawn_in_backend() {  # $1=inner (new-pane cmd) $2=title $3=named (bare-shell named-session cmd)
  ...
  # WezTerm: a new tab, via the cli that can actually reach the GUI. From WSL that is
  # wezterm.exe — the Linux cli sees no GUI there and would quietly daemonize its OWN headless
  # wezterm-mux-server and spawn into it, where nobody is attached, which is how CTRL+N read as
  # doing nothing at all. `bash` still resolves WSL-side because --domain-name pins the spawn to
  # the WSL domain (unlike the Windows spawns above, which want $WIN_GITBASH in `local`); the
  # pin also keeps this correct if wezterm.lua's default_domain ever moves. No reachable cli
  # falls through to the named launcher below — in place is worse than a tab, but it is visible.
  if [ -n "${WEZTERM_PANE:-}" ] && av_wezterm_bin; then
    local -a dom=()
    if [ -n "${WSL_DISTRO_NAME:-}" ]; then dom=(--domain-name "$WSL_DOMAIN"); fi
    "$_wezbin" cli --no-auto-start spawn "${dom[@]}" -- bash -lc "$inner" >/dev/null 2>&1
    return 0
  fi
