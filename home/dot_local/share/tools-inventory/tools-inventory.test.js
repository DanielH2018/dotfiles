...
test("assets are interpolated without their trailing newline", () => {
  ...
  assert.ok(!/\n$/.test(render.asset(THEME_CSS)), "the shared theme must be interpolated without a trailing newline");
  assert.ok(render.asset(THEME_CSS).startsWith(":root{"), "the shared theme starts at the palette");
});
