...
def test_extracts_titles_pr_links_and_counts():
    with tempfile.TemporaryDirectory() as d:
        ...
        lines = [
            "not json",
            json.dumps({"type": "ai-title", "aiTitle": "Fix flaky test",
                        "timestamp": "2026-01-01T00:00:00Z"}),
            json.dumps({"type": "custom-title", "title": "My Custom Title",
                        "timestamp": "2026-01-01T00:01:00Z"}),
            json.dumps({"type": "pr-link", "url": "https://github.com/org/repo/pull/42",
                        "timestamp": "2026-01-01T00:02:00Z"}),
            json.dumps({"type": "user", "message": {"content": "Please fix the bug"},
                        "timestamp": "2026-01-01T00:03:00Z", "gitBranch": "feature/fix-bug"}),
        ...
        write(os.path.join(session_dir, "-workspace", "chat.jsonl"), "\n".join(lines) + "\n")
...
def test_summarize_without_api_key_fails_closed():
    with tempfile.TemporaryDirectory() as d:
        ...
        write(json_file, json.dumps({"user_messages": ["hi"], "commits": "", "titles": []}))
...
def test_summarize_with_no_content_skips_network():
    with tempfile.TemporaryDirectory() as d:
        ...
        rc, out, err = run_summarize(json_file, extra_args=["--api-key=fake-key-for-test"])
        ...
        assert data == original, "data should pass through unchanged when there is nothing to summarize"
