-- In-editor SQL/query runner: visual-select a query, pipe it to a registered
-- backend command (psql, mysql, athena-query.sh, …), show results in a split.
-- Backends persist to stdpath('data')/sql-runner/commands.json, so they survive
-- restarts and are picked interactively via vim.ui.select.
--
-- Adapted from hendrikmi/dotfiles (nvim/lua/tools/sql-runner.lua). Backend-agnostic:
-- whatever you register just has to read the selection on stdin and print rows.
-- Windows note: the run step uses `:'<,'>write !<cmd> > file`, which runs through
-- nvim's 'shell'. options.lua forces cmd.exe on win32, so register backends with
-- cmd.exe-valid syntax there (a bare `psql "$DB"`-style env expansion won't expand).
--
-- Commands:  :RunSQL (visual)  :SelectSqlCmd  :AddSqlCmd  :RemoveSqlCmd
-- Keymaps:   <leader>pr run · <leader>ps select · <leader>pa add · <leader>px remove
