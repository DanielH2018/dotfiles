...
const SRC = path.join(__dirname, '..', 'home', 'dot_local', 'bin', 'executable_steam');
const body = fs.readFileSync(SRC, 'utf8');

const DESKTOP = path.join(
  __dirname, '..', 'home', 'dot_local', 'share', 'applications', 'steam.desktop.tmpl',
);
...
function run({ mullvadExclude, args = [], realExecutable = true, env = {} } = {}) {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), 'steam-wrapper-'));
  dirs.push(home);
